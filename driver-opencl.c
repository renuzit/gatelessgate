...
static void opencl_thread_shutdown(struct thr_info *thr)
{
    ...
    if (clState) {
        ...
        if (clState->buffer1) clReleaseMemObject(clState->buffer1);
        if (clState->buffer2) clReleaseMemObject(clState->buffer2);
        if (clState->buffer3) clReleaseMemObject(clState->buffer3);
        if (clState->buffer4) clReleaseMemObject(clState->buffer4);
        if (clState->buffer5) clReleaseMemObject(clState->buffer5);
        if (clState->buffer6) clReleaseMemObject(clState->buffer6);
        if (clState->buffer7) clReleaseMemObject(clState->buffer7);
        if (clState->buffer8) clReleaseMemObject(clState->buffer8);
        if (clState->buffer9) clReleaseMemObject(clState->buffer9);
        if (clState->buffer10) clReleaseMemObject(clState->buffer10);
        if (clState->buffer11) clReleaseMemObject(clState->buffer11);
