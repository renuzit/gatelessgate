...
try:
    ...
    # so use module bundled with Gateless Gate
...
class silentarmy:
    ...
    def start_solvers(self, devid, threads):
        ...
        if banner != "Gateless Gate mining mode ready":
    ...
    def set_nonce_leftpart(self, n):
        ...
        if l > 17:
            # Gateless Gate requires the last 12 bytes to be zero, then 3 bytes
            # to vary the nonce, this leaves at most 17 bytes that can be
            # fixed by the server.
            fatal('Stratum: Gateless Gate is not compatible with servers ' +
    ...
    def stratum_msg(self, method, *args):
        ...
        if method == 'mining.subscribe':
            p = ["Gateless Gate", None, self.host, str(self.port)]
...
def main():
    ...
    print("Gateless Gate, a Zcash miner");
    print("Copyright 2016 zawawa @ bitcointalk.org");
    parser = OptionParser()
    parser.add_option(
            "-v", "--verbose",
