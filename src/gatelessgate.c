...
extern const double eth2pow256 = 115792089237316195423570985008687907853269984665640564039457584007913129639936.0;
...
static bool work_decode_eth(struct pool *pool, struct work *work, json_t *val, json_t *ethval2)
{
    ...
    //const char *BlockHeightStr, *NetDiffStr, FinalNetDiffStr[65];
    ...
    const char *EthWorkStr = json_string_value(json_array_get(res_arr, 0));

    const char *SeedHashStr = json_string_value(json_array_get(res_arr, 1));

    const char *TgtStr = json_string_value(json_array_get(res_arr, 2));
...
static void *stratum_sthread(void *userdata)
{
    ...
    while (42) {
        ...
        if (pool->algorithm.type == ALGO_ETHASH) {
            ...
            char *ASCIINonce = bin2hex((unsigned char *)&work->XMRNonce, 4);
